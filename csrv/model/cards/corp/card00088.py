...
class TraceForPowerCounter(trace.Trace):
  DESCRIPTION = 'Trace 3 - if successful, place 1 power counter on Card00088'
...
class TakeATag(actions.Action):
  ...
  def resolve(self, response=None, ignore_clicks=False, ignore_all_costs=False):
    self.game.runner.tags += 1
    self.card.log('The runner takes a tag from Card00088')
...
class Card00088(ice.Ice):
  ...
  WHEN_REZZED_PROVIDES_CHOICES_FOR = {
      ...
      timing_phases.CorpUseAbilities: 'card00088_abilities',
  }
