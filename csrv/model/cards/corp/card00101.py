...
class Card00101Cost(cost.RezIceCost):
  def can_pay(self):
  ...
  def pay(self, response=None, ignore_clicks=False):
    if not response or not response.agenda:
      raise errors.CostNotSatisfied('You must forfeit an agenda to rez Card00101')
...
class RezCard00101(actions.RezIce):
  COST_CLASS = Card00101Cost
  REQUEST_CLASS = parameters.ForfeitAgendaRequest


class Card00101(ice.Ice):
  ...
  def build_actions(self):
    ...
    self._rez_action = RezCard00101(self.game, self.player, self)
