...
class ChooseIce(timing_phases.BasePhase):
  """Choose a piece of ice for card00037."""
...
class Card00037Action(actions.Action):

  def __init__(self, game, player, card=None, card00037=None):
    actions.Action.__init__(self, game, player, card=card)
    self.card00037 = card00037
  ...
  def resolve(self, response=None, ignore_clicks=False, ignore_all_costs=False):
    ...
    self.card00037.modify(self.card)
  ...
  def modify_ice_actions(self):
    ...
    for server in self.game.corp.servers:
      for ice in server.ice.cards:
        actions.append(Card00037Action(
