...
class TrashCard(actions.Trash):
  ...
  def __init__(self, game, player, card=None, card00047=None):
    actions.Trash.__init__(self, game, player, card=card)
    self.card00047 = card00047
...
class Card00047(resource.Resource):
      ...
      for card in self.player.rig.cards:
        if not card == self:
          trash_actions.append(TrashCard(
              self.game, self.player, card=card, card00047=self))
